...
func (er *EthResender) resendUnconfirmed() error {
	...
	er.logger.Infow(fmt.Sprintf("Re-sending %d unconfirmed transactions that were last sent over %s ago. These transactions are taking longer than usual to be mined. %s", len(attempts), ageThreshold, static.EthNodeConnectivityProblemLabel), "n", len(attempts), "ethTxIDs", ethTxIDs, "hashes", hashes)
	...
	for i := 0; i < len(reqs); i += batchSize {
		...
		if err := er.ethClient.BatchCallContextAll(er.ctx, reqs[i:j]); err != nil {
